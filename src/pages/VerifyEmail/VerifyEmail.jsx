...
export default function ResetPasswordForm() {
  ...
  return (
    <div className="w-full h-full max-w-md flex flex-col justify-center  px-2 md:px-6 lg:p-6">
      <div className="text-center">
        <img src="/logoIcon.png" alt="Logo" className="w-24 h-24 mx-auto flex items-center justify-center" />
        <h2 className="text-2xl lg:text-3xl font-semibold mb-6">Verify your email</h2>
        <p className="text-gray-500 mt-2">{successMessage}</p>
        <div className="flex justify-center items-center">
          <div className="border shadow-custom-light3 mt-4 mb-8 py-2 px-6 rounded-full">{userEmail}</div>
        </div>
      </div>
      <div className="flex items-center justify-center">
        <HStack>
          <PinInput onChange={(value) => setCode(value)} focusBorderColor="gray.500">
            <PinInputField />
            <PinInputField />
            <PinInputField />
            <PinInputField />
      ...
      <p className="text-gray-500 text-center text-sm mt-4">Enter the 6-digit code sent to your email</p>

      <button
        className={`bg-accent1 text-sm mt-10 text-secondary shadow transition-all hover:shadow-custom-light hover:bg-accent1Dark duration-300 ease-in-out font-semibold py-3 px-4 rounded-full focus:outline-none focus:shadow-outline w-full ${
      ...
      <div className="flex items-center text-sm justify-center gap-2 mt-auto">
        <p>Didn&apos;t receive the code in 1 minute?</p>
        <p className="cursor-pointer font-semibold" onClick={resendVerifyCode}>

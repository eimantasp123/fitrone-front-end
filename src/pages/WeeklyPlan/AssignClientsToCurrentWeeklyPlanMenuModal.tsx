import { fetchCurrentWeeklyPlanMenuDetails } from "@/api/weeklyPlanApi";
import ConfirmActionModal from "@/components/common/ConfirmActionModal";
import CustomButton from "@/components/common/CustomButton";
import EmptyState from "@/components/common/EmptyState";
import { useDynamicDisclosure } from "@/hooks/useDynamicDisclosure";
import { useDeleteClientOrGroupFromMenu } from "@/hooks/WeeklyPlan/useDeleteClientOrGroupFromMenu";
import { capitalizeFirstLetter } from "@/utils/helper";

import { useAssignClientsOrGroupToWeeklyPlanMenu } from "@/hooks/WeeklyPlan/useAssignClientsOrGroupToWeeklyPlanMenu";
import {
  Modal,
  ModalBody,
  ModalCloseButton,
...
interface AssignClientsToCurrentWeeklyPlanMenuModalProps {
  isOpen: boolean;
  onClose: () => void;
  weeklyPlanId: string;
...
/**
 * This component is a modal that allows the user to assign clients to a current weekly plan menu.
 */
const AssignClientsToCurrentWeeklyPlanMenuModal: React.FC<
  AssignClientsToCurrentWeeklyPlanMenuModalProps
> = ({ isOpen, onClose, weeklyPlanId, weeklyPlanMenuId }) => {
  const { t } = useTranslation(["weeklyPlan", "groups", "common"]);
  ...
  const { mutate: assignClientsOrGroupToWeeklyPlanMenu, isPending } =
    useAssignClientsOrGroupToWeeklyPlanMenu(() => {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: fetchCurrentWeeklyPlanMenuDetails,
  ...
  const assignClients = (clients: string[]) => {
    if (!weeklyPlanId || !weeklyPlanMenuId || !clients) return;
    assignClientsOrGroupToWeeklyPlanMenu({
      weeklyPlanId,
      weeklyPlanMenuId,
      objectId: clients,
      type: "client",
            ...
            {menuData && !isError && !isLoading && (
              <div className="grid-col-1 mt-4 grid gap-2 md:grid-rows-1 md:gap-0">
                ...
                <div className="min-h-[350px] w-full flex-col gap-3 md:min-h-[600px] lg:px-4">
                  ...
                  {menuData.menuDetails.assignedClients.length === 0 && (
                    <div className="px-2">
                      <div className="flex min-h-[350px] w-full flex-col items-center justify-center gap-1 rounded-lg border border-dashed border-primary bg-primaryLighter px-4 text-center dark:bg-primaryLighter md:min-h-[500px]">
...
export default AssignClientsToCurrentWeeklyPlanMenuModal;

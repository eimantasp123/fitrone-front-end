 ...
 */
const MealOverviewCard: React.FC<MealOverviewCardProps> = React.memo(
  ({ meal, setModalState, openDeleteModal }) => {
              ...
              <div className="flex w-full flex-wrap items-center gap-2 border-b-[1px] px-3 py-2 text-xs">
                ...
                <div className="flex flex-wrap gap-2">
                  <span className="rounded-full bg-backgroundSecondary px-[10px] py-[2px] text-textPrimary dark:bg-backgroundLight">
                    {
                      categories.find((item) => item.key === meal.category)
                        ?.title
                    }
                  </span>
                </div>
...
MealOverviewCard.displayName = "MealOverviewCard";

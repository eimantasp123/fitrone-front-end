...
interface AssignMealForCurrentDayModalHeaderProps {
  t: TFunction;
 ...
 */
const AssignMealForCurrentDayModalHeader: React.FC<
  AssignMealForCurrentDayModalHeaderProps
    ...
    <div className="grid grid-rows-6 gap-[6px] md:grid-cols-8 md:grid-rows-3 md:gap-3 xl:grid-rows-2">
      ...
      <div className="row-span-1 mt-[1px] flex items-center gap-3 md:col-span-8 xl:col-span-2">
        <CustomButton
          type="lightSecondary"
          onClick={() =>
            setFilters({ preference: null, restriction: null, category: null })
          }
          text={t("meals:resetFilters")}
          widthFull={true}
        />
        <CustomButton widthFull={true} onClick={() => {}} text={t("search")} />
      </div>
    </div>
  );
};

export default AssignMealForCurrentDayModalHeader;
